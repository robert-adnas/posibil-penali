...
export function ChangeLogList({
  ...
  from,
  fromLabel,
  emptyText = 'Nu exista inca modificari consemnate in jurnalul editorial.',
}) {
  if (!items.length) {
            ...
            >
              <div className="change-log-main">
                <div className="change-log-topline">
                  <span className="change-log-kind">{CHANGE_KIND_LABELS[entry.kind] || 'Modificare'}</span>
                  <span className="detail-panel-status change-log-status" data-status={entry.politician.status}>
                ...
                <span className="change-log-meta">
                  {entry.politician.party}{' \u00b7 '}{POSITION_LABELS[entry.politician.position_type] || entry.politician.position_type}
                </span>
